...
from config import Config, ColorScheme
...
class SettingsPanel:
    ...
    def _draw_info(self, screen):
        ...
        # Duration estimate. The app runs with no inter-sequence pause, so a
        # selection is just num_sequences blocks of 4 flashes at one SOA each.
        num_seq = self._values.num_sequences
        duration_ms = 4 * num_seq * soa
